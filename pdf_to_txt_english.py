...
def pdf_to_text(pdf_path, output_txt):
    ...
    with open(pdf_path, 'rb') as pdf_file:
        ...
        for page_num in range(1, 20):
            ...
            if any(keyword in pre_page_text for keyword in [". . . . ", "...............", "LIST OF ABBREVIATIONS", "Table"]):
                continue
            ...
            for i in range(len(lines)):
                if len(lines[i]) > 2:
                    ...
                    if word_count <= LIMIT:
                        continue
                    if ((lines[i][0] >= '0' and lines[i][0] <= '9') and (lines[i][1] == '.') and (lines[i][2] >= '0' and lines[i][2] <= '9')) or (lines[i][: 7] == 'CHAPTER') or (lines[i][: 6] == 'Figure'):
                        continue
        ...
        for page_num in range(21, len(pdf_reader.pages)):
            ...
            # Nếu gặp bảng thì skip cả trang
            if any(keyword in pre_page_text for keyword in ["Table"]):
                continue
            
            if any(keyword in pre_page_text for keyword in ["REFERENCE"]):
                break
...
if __name__ == "__main__":
    input_dir = '/Users/hoangducanh/Library/Mobile Documents/com~apple~CloudDocs/Hoc o HUST/Nhom anh Minh/pdf to text/test input/english/'
    output_dir = '/Users/hoangducanh/Library/Mobile Documents/com~apple~CloudDocs/Hoc o HUST/Nhom anh Minh/pdf to text/test output/english/'
