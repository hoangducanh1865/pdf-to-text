...
def pdf_to_text(pdf_path, output_txt):
    ...
    with open(pdf_path, 'rb') as pdf_file:
        ...
        for page_num in range(1, 20):
            ...
            if found_count < 2:
                if 'CHAPTER 1' in pre_page_text:
                    found_count += 1
                    if found_count == 0 or found_count == 1:
                        continue
                ...
                if len(lines[i]) > 2:
                    ...
                    if ((lines[i][0] >= '0' and lines[i][0] <= '9') and (lines[i][1] == '.') and (lines[i][2] >= '0' and lines[i][2] <= '9')) or (lines[i][: 7] == 'CHAPTER') or (lines[i][: 6] == 'Figure'):
                        ...
                        if lines[i][-1] == '-' and lines[i][-2] >= 'a' and lines[i][-2] <= 'z':
                            page_text += clean_line(lines[i][:-1])
                        else:
                            if lines[i][-1] == '.' or lines[i][-1] == ':':
                                page_text += clean_line(lines[i]) + '\n' 
                            else:
                                page_text += clean_line(lines[i]) 
...
if __name__ == "__main__":
    ...
    output_dir = '/Users/trannguyenmyanh/Documents/HUST/AUTH SCAN/pdf-to-text/test output/'
